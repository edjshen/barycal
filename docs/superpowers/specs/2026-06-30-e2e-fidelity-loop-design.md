...
> **barycal**, **plur-nyc**, and **poisys** — that measures the _fidelity_ of each repo's
...
**Goal.** Raise the _fidelity_ of user-simulation E2E testing in three repos, automatically and
continuously, by closing two gaps in priority order:

1. **Trust gap (first).** Many specs are _soft_: they pass even when the feature is broken.
   ...
   there means nothing. The loop's first job is to make a green run _mean the flow works_.
...
**Why a scorecard + mutation check.** "Self-learning" is only real if the loop has _memory_ and a
_monotonic target_. The spine is a small per-repo **fidelity scorecard** (JSON). The headline
...
- **Diff surface is an explicit allowlist ONLY** — the loop **must abort the PR if the diff touches
  ...
  - `.learned-experience/**`
  - `playwright.config.*` and dedicated e2e helper/fixture/seed files _inside the test dir_
...
- **Never merge, never deploy, never push to `main`.** The PR is the terminal output. The MoE panel
  is a _pre-PR quality gate_; it does **not** authorize shipping. When the human later merges a
...
3. **Improve (bounded, top 3–5).** Fix in priority order:
   - _Trust:_ replace swallow-and-log with web-first assertions on the actual user-visible outcome;
     delete conditional-skips; replace `waitForTimeout`-for-state with `expect.poll`/web-first waits.
   - _Realism (only after a flow's trust is fixed):_ realistic personas/data, real navigation,
...
- Body (templated): scorecard **delta table**, ledger excerpt, **mutation evidence**, MoE panel
  verdicts, and the explicit line _"Auto-generated by e2e-fidelity-loop. Not authorized to merge or
  deploy — human review + standard deploy gate required."_
...
- Mechanism: `create_trigger` with `create_new_session_on_fire=true` + a 5-field cron expression
  ...
  standalone instruction: _"Run the e2e-fidelity-loop for <repo> per its `.learned-experience/
e2e-contract.md`; open one reviewed PR; never merge or deploy."_
...
```json
{
  ...
  "delta_vs_prev": { "soft_tests": -3, "flows_asserted": +2, "mutation_kill_rate": +0.1 },
...
- **Seed flow inventory:** landing/auth, navigation, discover, calendar, create-event, circles,
  ...
  existing `e2e/0x–15` specs — most are currently _soft_ and need trust fixes first).
...
- **Note:** plur-nyc's suite is _mid-build_; here the loop both finishes coverage and raises fidelity.
...
- _ponytail ceiling:_ three committed copies can drift. Acceptable for v1; if drift bites, promote
...
barycal first, then U8 propagates** — this _is_ "build once, then instantiate," consistent with the
chosen rollout.
