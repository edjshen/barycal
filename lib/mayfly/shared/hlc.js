...
// Reject implausibly-future remote wall-clocks when observing. An authenticated
// member could otherwise stamp a frame far in the future and pin every peer's
// clock there (ordering vandalism). 5 min covers real device skew; anything
// beyond is clamped to now+skew rather than trusted.
const MAX_OBSERVE_SKEW_MS = 5 * 60 * 1000;
...
export class HybridLogicalClock {
  ...
  observe(remote) {
    const physical = Date.now();
    // Clamp an implausibly-future remote stamp so it can't pin our clock ahead.
    const remoteWall = Math.min(remote.wallMillis, physical + MAX_OBSERVE_SKEW_MS);
    const maxWall = Math.max(physical, this.lastWall, remoteWall);
    if (maxWall === this.lastWall && maxWall === remoteWall) {
