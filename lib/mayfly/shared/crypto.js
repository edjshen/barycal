 ...
 * the ciphertext PLUS routing metadata (roomId, message id, sender pubkey, hlc,
 * kind) so order, identity, and origin are tamper-evident. `id` is included so a
 * malicious relay cannot re-`id` and re-deliver an authentic message to defeat
 * the client's id-based dedupe (the signature would no longer verify). Both
 * signer and verifier must build this identically — do not reorder fields.
 */
