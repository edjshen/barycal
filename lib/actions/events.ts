...
type EventPatch = { type?: string; title?: string; description?: string; location?: string; startTime?: string; endTime?: string | null; recurring?: string | null; allDay?: boolean; color?: string | null; visibility?: string; };
...
function splitOccurrenceId(eventId: string): { baseId: string; dateKey: string | null } {
  if (eventId.includes('__')) { const [b, d] = eventId.split('__'); return { baseId: b, dateKey: d }; }
...
function mergeContent(base: BarycalEvent, patch: EventPatch) {
  const title = patch.title !== undefined ? clampStr(patch.title, LIMITS.title).trim() || base.title : base.title;
  return {
    type: patch.type !== undefined ? oneOf(patch.type, EVENT_TYPES, base.type) : base.type,
    title,
    description: patch.description !== undefined ? clampStr(patch.description, LIMITS.bio) : base.description,
    location: patch.location !== undefined ? clampStr(patch.location, LIMITS.location) : base.location,
    startTime: patch.startTime !== undefined ? toISOOrThrow(patch.startTime, 'start time') : base.startTime,
    endTime: patch.endTime !== undefined ? (patch.endTime ? toISOOrThrow(patch.endTime, 'end time') : null) : base.endTime,
    allDay: patch.allDay !== undefined ? !!patch.allDay : base.allDay,
    color: patch.color !== undefined ? normColor(patch.color) : base.color,
    visibility: patch.visibility !== undefined ? oneOf(patch.visibility, VIS, base.visibility) : base.visibility,
  };
}
const revalidateAll = () => { revalidatePath('/plans'); revalidatePath('/discover'); revalidatePath('/calendar'); };
...
export async function updateEvent(eventId: string, patch: EventPatch, opts?: { scope?: RecurScope }) {
  const uid = await requireUserId();
  ...
  if (base.recurring && scope === 'single' && dateKey) {
    ...
    const existing = (await getDb().select().from(events).where(and(eq(events.parentId, baseId), eq(events.originalDate, dateKey))).limit(1))[0];
    if (existing) {
      await getDb().update(events).set({ ...content, cancelled: false }).where(eq(events.id, existing.id));
      revalidateAll();
      return { id: existing.id };
    }
    const id = crypto.randomUUID();
    await getDb().insert(events).values({ id, creatorId: uid, ...content, recurring: null, parentId: baseId, originalDate: dateKey, cancelled: false, recurUntil: null, expiresAt: null, createdAt: nowISO });
    await getDb().insert(attendance).values({ id: crypto.randomUUID(), eventId: id, userId: uid, rsvp: 'going', createdAt: nowISO });
    revalidateAll();
    return { id };
  }
  ...
  if (base.recurring && scope === 'following' && dateKey) {
    ...
    await getDb().delete(events).where(and(eq(events.parentId, baseId), gte(events.originalDate, dateKey)));
    const id = crypto.randomUUID();
    await getDb().insert(events).values({ id, creatorId: uid, ...content, recurring: normRecur(patch.recurring !== undefined ? patch.recurring : base.recurring), parentId: null, originalDate: null, cancelled: false, recurUntil: null, expiresAt: null, createdAt: nowISO });
    await getDb().insert(attendance).values({ id: crypto.randomUUID(), eventId: id, userId: uid, rsvp: 'going', createdAt: nowISO });
    revalidateAll();
    return { id };
  }
  ...
  if (patch.visibility !== undefined) set.visibility = oneOf(patch.visibility, VIS, base.visibility);
...
export async function deleteEvent(eventId: string, opts?: { scope?: RecurScope }) {
  ...
  if (base.recurring && scope === 'following' && dateKey) {
    await getDb().update(events).set({ recurUntil: occurrenceStartISO(base, dateKey) }).where(eq(events.id, baseId));
    await getDb().delete(events).where(and(eq(events.parentId, baseId), gte(events.originalDate, dateKey)));
    revalidateAll();
    return;
  }
  ...
  if (base.recurring && scope === 'single' && dateKey) {
    const existing = (await getDb().select().from(events).where(and(eq(events.parentId, baseId), eq(events.originalDate, dateKey))).limit(1))[0];
    if (existing) await getDb().update(events).set({ cancelled: true }).where(eq(events.id, existing.id));
    else await getDb().insert(events).values({
      id: crypto.randomUUID(), creatorId: uid, type: base.type, title: base.title, description: '', location: '',
      startTime: occurrenceStartISO(base, dateKey), endTime: null, recurring: null, allDay: base.allDay, color: base.color,
      parentId: baseId, originalDate: dateKey, cancelled: true, recurUntil: null, visibility: base.visibility, expiresAt: null, createdAt: nowISO,
    });
    revalidateAll();
    return;
  }
  ...
  const children = await getDb().select({ id: events.id }).from(events).where(eq(events.parentId, baseId));
