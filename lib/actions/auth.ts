...
async function underLimit(scope: string, key: string, max: number, windowMs: number): Promise<boolean> {
  try {
...
export async function login(_prev: AuthState, form: FormData): Promise<AuthState> {
  ...
  if (handle && !(await underLimit('auth.login.handle', handle, 10, TEN_MIN))) return { error: TOO_MANY };

  const u = handle ? (await getDb().select().from(users).where(eq(users.handle, handle)).limit(1))[0] : undefined;
  if (!u) {
...
export async function register(_prev: AuthState, form: FormData): Promise<AuthState> {
  ...
  const displayName = (String(form.get('displayName') || '') || handle).slice(0, LIMITS.displayName);
  if (!handle || !password) return { error: 'Username and password required' };
  if (password.length > LIMITS.password) return { error: `Password must be at most ${LIMITS.password} characters` };
