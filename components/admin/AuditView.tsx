...
export default function AuditView({ rows, page, action }: { rows: Row[]; page: number; action: string }) {
  const q = (p: number) => `/admin/audit?page=${p}${action ? `&action=${encodeURIComponent(action)}` : ''}`;
  return (
    <div>
      <table>
        <thead><tr><th scope="col">When</th><th scope="col">Action</th><th scope="col">Target</th><th scope="col">Summary</th></tr></thead>
        <tbody>{rows.map((r) => (
